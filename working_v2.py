...
if __name__ == "__main__":
    with gr.Blocks(title="AI Twin", theme=gr.themes.Ocean) as demo:
        gr.Markdown("AI Twin")
        with gr.Row():
            with gr.Column():
                json_message = gr.Textbox(
                label="Json Message",
                value="{\"containerTrackingId\" : \"abcd-1\",  \"location\":\"door101\", \"item\":334343,\"quantity\":5, \"status\":\"AVAILABLE\"}"
                )
        ...
        with gr.Row():
            with gr.Column():
                ...
                label="Input",
                value="Read from neo4j where containerTrackingId = 'abcd-1' and detect anomalies in the data."
                )
        ...
        with gr.Row():
            with gr.Column():
                ...
                label="Input",
                value="For the containerTrackingId = 'abcd-2' in neo4j, create an Order Node with orderId = 'order-2' and link it to the Container Node."
                )
        ...
        submit_btn.click(
            ...
            inputs=[json_message],
            outputs=output_box,
            api_name="chat",show_progress='minimal'
        )
        detect_anomaly_submit_btn.click(
            ...
            api_name="chat",show_progress=False
        )
